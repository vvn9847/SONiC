...
## 1. Scope
This document describes the high level design of the software pipeline infrastructure for Orchagent and Syncd, also inculdes the apporch to solve bottleneck caused by redis I/O operations in APPL_DB.  
...
1. A TCP packet arrives at bgp's socket in kernel space. Kernel delivers the payload to bgpd process.
...
3. Zebra decodes the message from bgpd, and delivers this netlink-route message to fpmsyncd.
4. Fpmsyncd processes the netlink message and pushes this state into APPL_DB.
5. Being orchagent an APPL_DB subscriber, it consumes the routing information pushed to APPL_DB.
6. After processing the received information, orchagent will invoke sairedis APIs to push the route information into ASIC_DB.
7. Being syncd an ASIC_DB subscriber, it will receive the new state generated by orchagent.
8. Syncd will process the information and invoke SAI APIs to inject this state into the corresponding asic-driver. Finally new route is pushed to ASIC.
...
Futhermore, orchagent performs a series of operations in *process 6*, as show in figure 2.
<!-- omit in toc -->
...
- Provide a software pipeline infrastructure for orchagent
- Provide a software pipeline infrastructure for syncd
- Pipeline infrastructure should support the warm restart operations 
- Pipeline infrastructure should maintain the time sequence of route loading
- Provide a flush timer in fpmsyncd for flushing routes in a large batch size 
...
The figure below shows current performance on Alibaba platform in an end to end process.
<p align=center>
